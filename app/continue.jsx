...
import { View, StyleSheet, TextInput, Alert, Button, FlatList, TouchableOpacity, Text } from 'react-native';
...
export default function ContinuePage() {
  ...
  const [searchedLocation, setSearchedLocation] = useState({ latitude: null, longitude: null });
  ...
  async function getPlaces(query) {
    ...
    try {
      const response = await axios.get(
        ...
        {
          params: {
            access_token: 'pk.eyJ1IjoiY3Jpc3RpYnVsYXQiLCJhIjoiY20xNWNhbnZkMDZtdzJycXZoOHpxYTlkbSJ9.HhGmVWTMZa9r1-dOL0RPXQ',
          },
        }
      );
      ...
      Alert.alert("Error", "There was an error while fetching places. Please try again.");
      setSearchResults([]);
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "There was an error submitting your location. Please try again.");
    }
  };
        ...
        >
          {searchedLocation.latitude && searchedLocation.longitude && (
            <>
              ...
              />
              <Circle
                center={{ latitude: searchedLocation.latitude, longitude: searchedLocation.longitude }}
                radius={100} 
                strokeWidth={2}
